...
export default function ProductsDemo() {
  ...
  const saveProduct = async () => {
    ...
    if (product.idEmpleado) {
      ...
      const url = _product.idEmpleado ? `http://localhost:8080/employee/${_product.idEmpleado}` : 'http://localhost:8080/employee';

      try {
        await apiClient[method.toLowerCase()](url, _product);
        fetchEmployees();
  ...
  const deleteProduct = async () => {
    if (product.idEmpleado) {
      try {
        await apiClient.del(`http://localhost:8080/employee/${product.idEmpleado}`);
  ...
  const activateEmpleado = async (id) => {
    try {
      ...
      fetchEmployees();
      toast.current.show({ severity: 'success', summary: 'Successful', detail: 'employee Activada', life: 3000 });
    } catch (error) {
  ...
  const activateSelectedEmpleados = async () => {
    if (selectedProducts && selectedProducts.length > 0) {
      try {
        ...
        setSelectedProducts(null);
        fetchEmployees();
        toast.current.show({ severity: 'success', summary: 'Successful', detail: 'employee Activadas', life: 3000 });
      } catch (error) {
